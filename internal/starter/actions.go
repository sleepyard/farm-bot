...
type locateOpts struct {
	quiet      bool // 屏态探测：不打未命中日志、不存 miss 图、不做全屏回退
	noPreview  bool
	noFallback bool // 大模板禁止全屏回退（横幅等）
}
...
func (n *navigator) clickTemplateOpts(rel, label string, roi vision.ROI, threshold float64, opt locateOpts) bool {
	...
	n.step(fmt.Sprintf("已点击 %s (%s)", label, rel))
...
func (n *navigator) detectEventLanding() bool {
	n.step("==========正在检测当前是否已经在新手套牌对决的活动页面==========")
	ok := n.visible("Buttons/event_title.png", roiEventTitle, 0.90)
	if ok {
		n.step("已在新手套牌对决活动页面")
	} else {
		n.step("当前不在新手套牌对决活动页面")
	}
	return ok
}
...
func (n *navigator) dismissClaimPopup() {
	...
	if n.clickTemplate("Buttons/claim.png", "领取奖励", roiRewardClaim, 0.85) {
		n.step("已关闭奖励弹窗")
...
func (n *navigator) clickAllFilter(inProg vision.Point, haveIP bool) {
	if haveIP {
		y := inProg.Y - 43
		if y < 40 {
			y = 40
		}
		_ = input.ClickClient(n.sess.HWND, inProg.X, y)
		n.step(fmt.Sprintf("几何点击「全部」@(%d,%d)", inProg.X, y))
		return
	}
	_ = input.ClickClient(n.sess.HWND, ptAllFilter.X, ptAllFilter.Y)
	n.step(fmt.Sprintf("参考点点击「全部」@(%d,%d)", ptAllFilter.X, ptAllFilter.Y))
}

func (n *navigator) findAndClickBanner() bool {
	const conf = 0.72
	// 横幅模板 ~446×177：禁止全屏回退（一次全屏曾要 60s+）
	opt := locateOpts{noFallback: true}
	if n.clickTemplateOpts("assert/starter_deck.PNG", "新手套牌对决横幅", roiStarterBanner, conf, opt) {
		return true
	}
	cx := roiStarterBanner.X + roiStarterBanner.W/2
	cy := roiStarterBanner.Y + roiStarterBanner.H/2
	for i := 0; i < 8; i++ {
		_ = input.WheelClient(n.sess.HWND, cx, cy, -360)
		if n.sleep(350 * time.Millisecond) {
			return false
		}
		if n.clickTemplateOpts("assert/starter_deck.PNG", "新手套牌对决横幅", roiStarterBanner, conf, opt) {
			n.step(fmt.Sprintf("滚动 %d 次后找到横幅", i+1))
			return true
		}
	}
	return false
...
func (n *navigator) pressEventPlay() bool {
	if n.clickTemplate("Buttons/event_play.png", "Play", roiEventPlay, 0.90) {
		n.step("已从活动页通过 Play 排队")
